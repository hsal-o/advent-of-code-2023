...
def main():
    ...
    for y in range(0, len(grid)):
        ...
        max_energized_count = max(max_energized_count, get_energized_count(grid, y, 0, Direction.RIGHT))
        # Scan from Right to Left
        max_energized_count = max(max_energized_count, get_energized_count(grid, y, len(grid[0])-1, Direction.LEFT))

    for x in range(0, len(grid[0])):
        # Scan from Top To Bottom
        max_energized_count = max(max_energized_count, get_energized_count(grid, 0, x, Direction.DOWN))
        # Scan from Bottom to Top
        max_energized_count = max(max_energized_count, get_energized_count(grid, len(grid)-1, x, Direction.UP))

    print(f"max_energized_count: {max_energized_count}")
